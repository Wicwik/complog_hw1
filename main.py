...
# Choose and uncommnet a sample test or specify vertices (numbers), edges (pairs of vertices) and colors (letters) 
...
for v in vertices:
	for c in colors:
		if c == colors[-1]:
			...
			f.write('color({0},{1}) V '.format(v, c))
...
for e in edges:
	for c in colors:
		f.write('-color({0},{1}) V -edge({0},{2}) V - color({2},{1})\n'.format(e[0], c, e[1]))
	f.write('\n')

f.close()

print('Output has been written to {0}'.format(output_file))
